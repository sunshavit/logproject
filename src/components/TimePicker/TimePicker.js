...
const TimePicker = ({
  ...
  const handleQuickTime = timeFrame => {
    setSelectedTimeFrame({ ...timeFrame, fromDate: setFrom(timeFrame), toDate: Date.now() });
  };
  ...
  const onApply = timeFrame => {
    ...
    return setSelectedTimeFrame({ fromDate: from.date, toDate: to.date, type: 'timeRange' });
  };
  ...
  return (
    <Styled.TimePickerWrap>
      ...
      />
      <CSSTransition in={isOpen} timeout={300} classNames="alert" unmountOnExit appear>
        <Styled.TimePickerModal ref={dateSelectRef} tabIndex="0" onBlur={e => handleBlur(e)}>
          ...
          {eventDuration ? (
            ...
            <QuickTabs handleQuickTime={handleQuickTime} selectedTimeFrame={selectedTimeFrame} />
          )}
...
TimePicker.propTypes = {
  getTimeFrame: PropTypes.func.isRequired,
  primaryTime: PropTypes.object,
  dateFormat: PropTypes.oneOf(['dd.MM.yyyy', 'dd/MM/yyyy', 'MM/dd/yyyy', 'MM.dd.yyyy']),
  eventDuration: PropTypes.shape({
